...
class Curlhttp{
    ...
    //查询线路ip
    function select_route_ip(){
        //34服务器用route=1的线路、35服务器用route=2的线路
        $rout_result = mysql_query("select * from `web_Refresh_water_route` where `route` = 1 limit 1");
        $rout_row = mysql_fetch_assoc($rout_result);
        return "http://".$rout_row['IP'].":".$rout_row['port'];
    }
	//HTTP认证
	 ...
	 * @access public
	 */
	public function fetch_into_file($url, $fp, $ip=null, $timeout=10)
	{
		//需要获取的URL地址
		curl_setopt($this->ch, CURLOPT_URL,$url);
